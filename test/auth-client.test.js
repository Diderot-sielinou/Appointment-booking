...
describe('Authentication API (/auth-client)', () => {
  ...
  before(async () => {
    ...
    try {
      await query('DELETE FROM clients WHERE email = $1', [testUserEmail]);
    } catch (err) {
  ...
  describe('POST /register', () => {
    it('should register a new client successfully', async () => {
      ...
      assert(res.body.message, 'client registered successfully');
      assert(res.body.clientId, 'User object should be returned');
      assert(res.body.clientId.id, 'User ID should be returned');
      testClientId = res.body.clientId.id; // Save for later tests
    });
  ...
  describe('POST /login', () => {
    it('should login the registered user successfully and return a token', async () => {
      ...
      assert.strictEqual(res.body.client.id, testClientId);
